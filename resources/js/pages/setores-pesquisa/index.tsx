...
export default function SetoresPesquisaIndex({
    ...
    const handleSearch = (e: FormEvent) => {
        e.preventDefault();
        router.get('/setores-pesquisa', { search }, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    const handleDelete = (setorPesquisaId: number) => {
        if (
            confirm(
                'Tem certeza que deseja excluir este setor de pesquisa?'
            )
        ) {
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Busque setores de pesquisa</CardDescription>
                    </CardHeader>
                    <CardContent>
                                ...
                                <tbody>
                                    {setoresPesquisa.data.length === 0 ? (
                                        <tr>
                                            ...
                                            >
                                                Nenhum setor de pesquisa encontrado
                                            </td>
                                        </tr>
                                    ) : (
                                        setoresPesquisa.data.map((setorPesquisa) => (
                                            <tr
                        ...
                        {setoresPesquisa.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                    {setoresPesquisa.links.map((link, index) => {
                                        if (!link.url) {
                                            return (
                                                <span
                                                    key={index}
