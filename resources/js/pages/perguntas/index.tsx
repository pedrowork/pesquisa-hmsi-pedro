...
export default function PerguntasIndex({
    ...
    const handleMoveUp = (index: number) => {
        ...
        const currentOrder =
            newOrder[currentPergunta.cod] ?? currentPergunta.ordem ?? index + 1;
        const previousOrder =
            newOrder[previousPergunta.cod] ?? previousPergunta.ordem ?? index;
    ...
    const handleMoveDown = (index: number) => {
        ...
        const currentOrder =
            newOrder[currentPergunta.cod] ?? currentPergunta.ordem ?? index + 1;
        const nextOrder =
            newOrder[nextPergunta.cod] ?? nextPergunta.ordem ?? index + 2;
    ...
    const handleDelete = (perguntaId: number, totalPesquisas: number) => {
        if (totalPesquisas > 0) {
            const message = t('messages.deleteQuestionWithSurveys', {
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            {t('common.total')}: {perguntas.total}{' '}
                            {t('questions.surveysCount')}
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                        ...
                                        perguntas.data.map((pergunta) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-4 py-3 text-sm text-muted-foreground">
                                                    {pergunta.total_pesquisas ||
                                                        0}{' '}
                                                    {t(
                                                        'questions.surveysCount',
                                                    )}
                                                </td>
                                                <td className="px-4 py-3">
                                                    <div className="flex justify-end gap-2">
                                                        <Can permission="perguntas.edit">
                                                            <Link
                                                                href={`/perguntas/${pergunta.cod}/edit`}
                                                            >
                                                                <Button
                                                                    variant="outline"
                                                                    size="sm"
                        ...
                        {perguntas.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                <div className="text-sm text-muted-foreground">
                                    {t('common.page')} {perguntas.current_page}{' '}
                                    {t('common.of')} {perguntas.last_page}
                                </div>
                                <div className="flex gap-2">
